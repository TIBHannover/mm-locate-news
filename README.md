...
For BreakingNews dataset download the trained models from [here](https://tib.eu/cloud/s/fymNNnyPsioGHAt) and extract in ``` breakingnews/experiments/snapshots```.
...
## Inference
To get the output predictions for an input image and text pair download [this](https://tib.eu/cloud/s/jNqigK2LoCfGLmN) and put it in ```inference``` folder.
```bash
